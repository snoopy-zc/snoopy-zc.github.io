---
title: 【生物信息学】概述
date: 2026-07-19T00:00:00+08:00
author: Chen Z
cover: bioinfomatics.jpeg
images:
- bioinfomatics.jpeg
categories:
- 生物信息学
- 学习笔记
---

生物信息学是一个前言交叉领域，发展并应用计算机和计算机技术去研究生物学问题。（详情页持续更新...）
 

<!--more-->
## 1 在线课程


 ✅ 多伦多大学：生物信息学方法 (https://www.bilibili.com/video/BV1uMNE6EEcS)
- 搬运 https://www.coursera.org/learn/bioinformatics-methods-1
- 搬运 https://www.coursera.org/learn/bioinformatics-methods-2

 ✅ 北京大学：生物信息学 (https://www.bilibili.com/video/BV13t411G7oh)

 ✅ 教育部生物科学101计划重点平台：生物信息学 (https://bis.zju.edu.cn/bioinfo101/) <font color="red">（推荐）</font>



## 2 参考书籍

1. 陈铭《生物信息学》(第4版)科学出版社

2. 赵旻《医学生物信息学数据分析与应用》科学出版社 <font color="red">（推荐）</font>

2. Selzer, P. M., Marhöfer, R. J. & Koch, O. Applied Bioinformatics. (Springer International Publishing, Cham, 2018). doi:10.1007/978-3-319-68301-0.

3. Haubold, B. & Börsch-Haubold, A. Bioinformatics for Evolutionary Biologists: A Problems Approach. (Springer International Publishing, Cham, 2017). doi:10.1007/978-3-319-67395-0.


## 3 参考博文

《生物信息学》笔记 @想成为大佬的生物狗
https://mp.weixin.qq.com/s/uxO5GBQfmFizNpTlkJ63fA


## 4 生信Agent（进阶）


 [Science | 生信分析太繁琐？斯坦福造了个全自动AI科研Agent Biomini，不用敲代码直接用](https://www.bilibili.com/video/BV1BYNT6aEVB?spm_id_from=333.1245.0.0)




## 学习路径

- 生物学基础(1-2周)
    - 书籍:《分子生物学精要》📖
    - 视频:B站生信必备生物学基础课程🎬
    - 主要内容包括基因、转录组、蛋白质组等基本概念和原理哦～

- 💻 Linux(1-2周，重点⭐)
    - 视频:某站Linux生信实操教程、Linux命令行基础🎥
    - 书籍:《Linux命令行与shell脚本编程大全》📚
    - 在生信分析中，Linux是处理大量数据和运行分析工具的重要环境，一定要好好学！

- 🗄️ SQL(1-2周)
    - 书籍:《SQL必知必会》或《MySQL必知必会》📖
    - 网站:菜鸟教程SQL、W3cSchool💻
    - SQL语言能帮助提取生物数据库中的数据，进行基本的数据筛选和整合分析～

- 🐍 Python/R语言(3-4周，重点⭐)
    - 书籍:《Python for Data Analysis》、《R语言实战》、《R for Data Science》📚
    - 这是生信分析的核心技能！掌握Pandas、NumPy、ggplot2等库，用于处理高通量测序数据和绘制分析图表📊是面试中的重要加分项，建议重点学习哦！

- 生信分析工具(2-3周，重点⭐)
    - 有许多实用的生信分析工具，如BWA、GATK、STAR、DESeq2、edgeR等🔧
    - 推荐学习这些工具的使用方法和实战应用，它们在序列比对、变异检测、差异分析等方面发挥重要作用，超实用！

- 📝 生信分析思路(长期)
    - 书籍:《生信分析实战》《高通量测序技术与应用》📖
    - 生信分析的基本步骤包括数据获取、质量控制、数据预处理、数据分析和结果可视化解读，慢慢积累就会懂～

- ✨ 项目实战(需求)
    - GEO数据库:包含大量基因表达数据，可用于练习数据挖掘和分析📊
    - TCGA数据库:提供癌症相关的多组学数据，适合开展癌症生信分析项目🔬

